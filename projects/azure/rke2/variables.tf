...
variable "instance_count" {
  description = "Specifies the number of Azure virtual machines instances (nodes) to create for the RKE2 cluster. This value defines the total cluster size, including the first server node, additional server nodes (if count <= 3), and worker nodes (if count > 3). Default is '1'."
  type        = number
  default     = 1
  validation {
